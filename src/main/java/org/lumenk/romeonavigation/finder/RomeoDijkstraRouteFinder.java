...
public class RomeoDijkstraRouteFinder {
    public static <WaypointIdType, RoadIdType, WaypointType extends OnRoadWaypoint<WaypointIdType, RoadIdType>, RoadType extends DistanceRoad<WaypointIdType, RoadIdType>> RomeoRouteInformation<WaypointIdType, RoadIdType> find(WaypointIdType departure, WaypointIdType arrival, DistanceRoadMapData<WaypointIdType, RoadIdType, DistanceRoad<WaypointIdType, RoadIdType>> map){
        ...
        while(true){
            ...
            while(queue1.getCount() > 0){ //큐에 있는 모든 검토 대상에 대하여 살펴보자.
                ...
                if(d < minValue){ //만약 계산한 거리가 이전까지 계산한 최소 거리보다 작다면
                    ...
                    current = temp; //이 도로에 대하여 다른 도로들과 비교를 수행할 것이다.
